...
namespace mfw::core
{
	...
	MFW_CORE_API void MFW_CORE_CALL func_info::clear()
	{
		name.clear();
		arg_infos.clear();
		return_info.clear();
	...
	MFW_CORE_API void MFW_CORE_CALL class_info::clear()
	{
		super::clear();
		name.clear();
		variables.clear();
	...
	MFW_CORE_API const class_info::member_variable_info * MFW_CORE_CALL class_info::get_variable(const ucstring_view &_name_) const
	{
		for(const member_variable_info &it : variables) {
			if(it.get_name() == _name_) {
				return &it;
			}
		}
		return nullptr;
	...
	MFW_CORE_API const func_info * MFW_CORE_CALL class_info::get_function(const ucstring_view &_name_) const
	{
		for(const func_info &it : functions) {
			if(it.get_name() == _name_) {
				return &it;
			}
		}
	...
	MFW_CORE_API void MFW_CORE_CALL class_info::member_variable_info::clear()
	{
		name.clear();
		offset = 0;
		var = nullptr;
